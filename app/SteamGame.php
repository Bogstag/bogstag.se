...
class SteamGame extends Model
{
    ...
    {
        $query->where('playtime_forever', '>', 0)->orderBy('playtime_2weeks', 'desc')->orderBy('playtime_forever',
                'desc');
    }
     ...
     * Returned games Ids is games that i have returned, but it still shows up in my stats.
     *
     * @return array
     */
    public static function getGamesWithNoStats()
    {
        $dynamicIds = self::select('id')->where('has_community_visible_stats', 0)->orwhere('playtime_forever',
                0)->pluck('id')->toArray();
